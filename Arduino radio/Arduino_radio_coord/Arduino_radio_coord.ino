...
char data = 5;    // Can range from 0  to 7 each number represents different wall configurations at a coordinate
char dir = 0;     // Can range from 0 to 3 each number represents a direction the robot is facing

unsigned long coord_info; // The combined coordinates and information that are sent through the radio
// Role management
//
...
void loop(void)
{
  ...
  if (role == role_ping_out)
  {
    ...
    // Add the X coordinates onto coord_info to be sent (x_coord takes up 4 bits)
    
   
    
    bool ok = radio.write( &coord_info, 25 );
